...
export default {
  async run(m, { sock, user, setting, isPartner, body }) {
    ...
    try {
      ...
      if (!isTag && m.quoted)
        isTag =
          m.quoted.sender === sock.user.decodedId ||
          m.quoted.sender === sock.user.decodedLid;
      if ((m.isGroup && isTag) || m.isPrivate) {
        ...
        if (!isPartner) {
          if (user.limit > 0) user.limit -= 1;
            ...
            return m.reply(
              `⚠️ Your limit is not enough to continue the chat, try \`${setting.prefixes[0]}claim\` command to claim limit.`,
            );
        }
