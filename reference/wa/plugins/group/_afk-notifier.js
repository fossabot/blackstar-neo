...
const notifySender = async (
  m,
  { sock, userData, senderData, setting, groupMetadata },
) => {
  const printNotifySender = frame(
    "AFK",
    [
...
export default {
  async run(m, { sock, db, user: senderData, setting, groupMetadata }) {
    if (m.fromMe) return;
    for (const userId of m.mentionedJid) {
      const userData = db.getUser(userId);
      if (!userData || isEmptyObject(userData.afkContext)) continue;
      notifySender(m, {
        sock,
        userData,
        senderData,
    ...
    if (m.quoted?.sender) {
      ...
      if (!userData || isEmptyObject(userData.afkContext)) return;
      notifySender(m, {
        sock,
        userData,
        senderData,
