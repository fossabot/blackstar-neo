...
export const handleWarning = async (
  m,
  { sock, participant, note, max = 5 } = {},
) => {
  if (++participant.warningPoint >= max) {
    const print = frame(
      "WARNING",
      [`👋🏻 Good bye!`, `*Warning*: ${max} / ${max}`],
      "⚠️",
    );
    ...
    const print = frame(
      "WARNING",
      [note, `*Warning*: ${participant.warningPoint} / ${max}`],
      "⚠️",
    );
    await sock.sendMessage(m.chat, {
      delete: {
...
export default {
  async run(m, { sock, group, isPartner, isAdmin, body }) {
    if (group.antiLink && !isPartner && !isAdmin && isURL(body)) {
    ...
    if (group.antiWALink && !isPartner && !isAdmin && isWhatsAppURL(body)) {
      const participant = group.participants[m.sender];
      handleWarning(m, {
        sock,
        participant,
