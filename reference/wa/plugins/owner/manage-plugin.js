...
export default {
  ...
  async run(m, { sock, isPrefix, command, text }) {
    if (!text) return m.reply(`👉🏻 *Example*: ${isPrefix + command} menu`);
    if (command === "delplugin" || command === "dp") {
      ...
      if (!pluginPath) return m.reply("❌ Plugin not found.");
      ...
      if (!qBody) return m.reply("💭 Reply plugin message.");
