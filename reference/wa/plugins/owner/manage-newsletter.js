...
export const isMeNewsletterAdmin = (id, data) =>
  data.find((newsletter) => newsletter.id === id).viewer_metadata.role !==
  "SUBSCRIBER";
...
export default {
  ...
  async run(m, { sock, setting, isPrefix, command, text }) {
    if (!setting.newsletterId || !isJidNewsletter(setting.newsletterId))
      return m.reply(
        `❌ Newsletter ID are still empty or invalid, you can change it with \`${isPrefix}setchid\` command.`,
      );
    ...
    if (command === "setchname") {
      ...
      if (!text)
        return m.reply(
          `👉🏻 *Example*: ${isPrefix + command} Weekly fresh updates`,
        );
