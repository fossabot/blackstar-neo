...
export default {
  ...
  async run(m, { sock, db, store, isPrefix, command, args }) {
    ...
    if (
      !q ||
      !q.text.includes(`You can choose group to manage the group's settings`)
    )
      return m.reply(
        `💭 Reply group lists message response from \`${isPrefix}groups\`.`,
      );
    ...
    if (!group) return m.reply("❌ Group not found.");
    const groupMetadata = store.getGroup(group.id);
    if (!groupMetadata) return m.reply("❌ Group metadata not found.");
    const participants = groupMetadata.participants;
    let isBotAdmin = false;
    for (
      let i = 0, participantsLength = participants.length;
    ...
    if (option === "close") {
      if (!isBotAdmin) return m.reply("❌ Bot is not an admin in that group.");
      ...
      await sock.sendText(group.id, "👋🏻 Good bye!", FAKE_QUOTE, {
        mentionAll: true,
      });
      ...
      sock.sendText(
        group.id,
        "📣 Bot muted in this group by bot staff.",
        FAKE_QUOTE,
      );
    } else if (option === "open") {
      if (!isBotAdmin) return m.reply("❌ Bot is not an admin in that group.");
      ...
      sock.sendText(
        group.id,
        "📣 Bot duration in this group has been reset and will stay forever.",
        FAKE_QUOTE,
      );
      ...
      sock.sendText(
        group.id,
        "📣 Bot unmuted in this group by bot staff.",
        FAKE_QUOTE,
      );
      ...
      await m.reply(
        `✅ Successfully add bot access for ${group.name} group and will expired at ${expiredAt}.`,
      );
      sock.sendText(
        group.id,
        `📣 Bot access in this group is valid until ${expiredAt}.`,
        FAKE_QUOTE,
      );
    } else m.reply(printHowTo(isPrefix, command));
  },
  partner: true,
};
