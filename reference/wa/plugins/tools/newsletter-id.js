...
export default {
  ...
  async run(m, { sock, isPrefix, command, args }) {
    try {
      if (!args[0])
        return m.reply(
          `👉🏻 *Example*: ${isPrefix + command} https://whatsapp.com/channel/0029VbBeIi33AzNKAeODXg16`,
        );
      m.react("🕒");
      const match = args[0].match(NEWSLETTER_URL_REGEX);
      if (!match) return m.reply("❌ Invalid URL.");
      const newsletterMetadata = await sock.newsletterMetadata(
        "INVITE",
        match[1],
      );
      const newsletterPicturePath =
        newsletterMetadata.thread_metadata?.preview?.direct_path;
      let newsletterPicture = newsletterPicturePath
        ? "https://pps.whatsapp.net" + newsletterPicturePath
        : botThumbnail;
      const print = frame(
        "CHANNEL INFO",
        [
