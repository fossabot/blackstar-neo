    ...
    if (command === "disk") {
      ...
      m.reply(
        "🗂️ *Disk Used*: " +
          ...
          " / " +
          formatSize(diskUsage.total),
      );
    } else if (command === "ping") {
      const old = performance.now();
      await m.react("🚀");
      const ping = (performance.now() - old) | 0;
      m.reply(getPingEmojis(ping) + " *Latency*: " + ping + "ms");
    } else if (command === "ram")
      m.reply(
        "💾 *RAM Usage*: " +
          formatSize(process.memoryUsage().rss) +
          " / " +
          formatSize(totalmem()),
      );
