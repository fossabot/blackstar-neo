...
export default async (gameName, gameConfig = {}) => {
  ...
  const gameCommand = {
    ...
    category: "games",
    async run(m, { sock, isPrefix, command, user }) {
      const gameSession = GameSession.get(m.chat);
      if (gameSession)
        return sock.sendText(
          m.chat,
          `❌ There is already a game running in this chat @${m.sender.split("@")[0]}.`,
          gameSession.chat,
        );
      ...
      const question =
        data[gameConfig.questionField] || gameConfig.questionField;
      const correctAnswer = data[gameConfig.answerField]?.toLowerCase();
      if (!correctAnswer) return m.reply("❌ Game correct answer not found.");
      let print = frame(
        gameConfig.title,
        [
          `*Question*: ${question}`,
  ...
  const clueCommand = {
    ...
    async run(m, { isPrefix }) {
      const gameSession = GameSession.get(m.chat);
      if (!gameSession)
        return m.reply(
          `❌ No game is currently running. Start a new one with: \`${isPrefix + gameConfig.command}\``,
        );
  ...
  const passCommand = {
    ...
    async run(m, { sock, isPrefix, user }) {
      const gameSession = GameSession.get(m.chat);
      if (!gameSession)
        return m.reply(
          `❌ No game is currently running. Start a new one with: \`${isPrefix + gameConfig.command}\``,
      ...
      if (user.limit >= gameSession.reward) {
        ...
        await m.reply(
          `✅ Successfully deleted *${toTitleCase(gameConfig.title.toLowerCase())}* game session. ${gameSession.reward} of your limit has been removed.`,
        );
      } else {
        user.limit = 0;
        await m.reply(
          `✅ Successfully deleted *${toTitleCase(gameConfig.title.toLowerCase())}* game session. All of your limit has been removed.`,
        );
      }
      GameSession.delete(m.chat);
      Processing.delete(m.chat);
  ...
  const gameEvent = {
    async run(m, { sock, body, user }) {
      ...
      if (similarity > 90) {
        ...
        GameSession.delete(m.chat);
      } else if (similarity > 70) await m.reply("🤏🏻 Almost there.");
    },
    group: true,
  };
